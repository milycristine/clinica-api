...
func (r *funcionarioRepository) CriarFuncionario(funcionario *models.Funcionario) error {
    ...
    INSERT INTO Funcionarios (
        ...
        TipoVinculo, PercentualRepasse, ValorFixoProcedimento, UnidadeId
    )
    VALUES (
        @Nome, @Sobrenome, @Cpf, @Rg, @DataNasc, @Email, @Senha, @Cargo, 
        @DateAdmissao, @HoraEntrada, @HoraSaida, @Salario, @Admin,
	...
	_, err = tx.Exec(funcQuery,
		...
		sql.Named("TipoVinculo", funcionario.TipoVinculo),
		sql.Named("PercentualRepasse", funcionario.PercentualRepasse),
		sql.Named("ValorFixoProcedimento", funcionario.ValorFixoProcedimento),
		sql.Named("UnidadeId", funcionario.UnidadeId),
	)
...
func (r *funcionarioRepository) ListarFuncionarios(page int) ([]models.Funcionario, error) {
	...
	query := `
		SELECT 
			...
			Status, ChavePix, Salario, Admin, TipoVinculo, PercentualRepasse, ValorFixoProcedimento, UnidadeId
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&f.Status, &f.ChavePix, &f.Salario, &f.Admin, &f.TipoVinculo, &f.PercentualRepasse, &f.ValorFixoProcedimento, &f.UnidadeId,
		); err != nil {
...
func (r *funcionarioRepository) EditarFuncionario(funcionario *models.Funcionario) error {
	...
	if funcionario.TipoVinculo != "" {
		setClauses = append(setClauses, "TipoVinculo = @TipoVinculo")
		args = append(args, sql.Named("TipoVinculo", funcionario.TipoVinculo))
	}
	if funcionario.PercentualRepasse != nil {
		setClauses = append(setClauses, "PercentualRepasse = @PercentualRepasse")
		args = append(args, sql.Named("PercentualRepasse", *funcionario.PercentualRepasse))
	}
	if funcionario.ValorFixoProcedimento != nil {
		setClauses = append(setClauses, "ValorFixoProcedimento = @ValorFixoProcedimento")
		args = append(args, sql.Named("ValorFixoProcedimento", *funcionario.ValorFixoProcedimento))
	}
